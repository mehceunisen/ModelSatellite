...
//void Quaternion_toEulerZYX(Quaternion* q, Quaternion* output)
//{
//    assert(output != NULL);
//
...
//    output->v[0] = atan2(sinr_cosp, cosr_cosp);
//
//    // Pitch (y-axis rotation)
//    double sinp = +2.0 * (q->w * q->v[1] - q->v[2] * q->v[0]);
//    if (fabs(sinp) >= 1)
//    	output->v[1] = copysign(M_PI / 2, sinp); // use 90 degrees if out of range
//    else
//    	output->v[1] = asin(sinp);
//
//    // Yaw (z-axis rotation)
//    double siny_cosp = +2.0 * (q->w * q->v[2] + q->v[0] * q->v[1]);
//    double cosy_cosp = +1.0 - 2.0 * (q->v[1] * q->v[1] + q->v[2] * q->v[2]);
//    output->v[2] = atan2(siny_cosp, cosy_cosp);
//}
